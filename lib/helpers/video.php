...
function hm_get_youtube_video_id_from_url($url) {
	preg_match('~watch\?v=([a-zA-Z0-9_-]*)~', $url, $video_id);

	return $video_id[1];
}

function hm_get_video_embed_code_by_url($url) {
	if ( preg_match('~youtube\.com~', $url) ) {
		return '<iframe width="100%" src="https://www.youtube.com/embed/' . hm_get_youtube_video_id_from_url( $url ) . '" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
	} else {
...
function hm_is_attachment_video($post_id) {
