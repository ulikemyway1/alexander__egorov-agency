...
function hm_get_svg_inline( $svg_path_or_url ) {
	$path = hm_convert_url_to_path( $svg_path_or_url );
	$svg  = file_get_contents( $path );

	return $svg;
}

function hm_svg_inline( $svg_path_or_url ) {
	echo hm_get_svg_inline( $svg_path_or_url );
}

function hm_get_svg_as_colorized_png($svg_path_or_url, $color = '') {
	// Look for the file in the theme cache
	$file_hash       = hash_file( 'md5', hm_convert_url_to_path($svg_path_or_url) );
	...
	// Generate the file
	$svg     = hm_get_svg_inline( $svg_path_or_url );
